...
**Just want to install the reader?** → [Back to the main README](../README.md)
for the download-and-run procedure. Everything below is how it was figured
...
of publicly available files. No proprietary source was used, and the **tools** read
firmware from the user's own device rather than shipping any.

Stock firmware images captured from the owner's own two units are archived under
[firmware/](firmware/), separately from the tooling — the BF07 appears to be out of
production with no working vendor update channel, so they are the only route back for
an owner who damages their device. See that directory's README; the rights holder can
open an issue to have them removed.
