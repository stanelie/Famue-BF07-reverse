...
| `This patch was NOT built for the firmware on this device` | Either your BF07 runs a firmware build this patch doesn't target, **or** a previous reader modified something this patch doesn't overwrite | Nothing was written — your device is fine. If you have installed a reader before, restore your original backup first (option 5) and try again. Otherwise open an issue with the sector addresses it printed. **Don't use `--force`** |
